(() => {
    ...
    function load() {
        try {
            ...
            const rows = Array.isArray(parsed.rows)
                ? parsed.rows.map(normalizeRow)
                : [makeEmptyRow()];
            return {
                rows: rows.length ? rows.slice(0, MAX_SLOTS) : [makeEmptyRow()],
                otherSection:
                    typeof parsed.otherSection === "string"
                        ? parsed.otherSection
                        : "",
        ...
        for (const c of cols) {
            ...
            if (c.label && c.label.includes(" ")) {
                ...
                if (lastSpace > 3 && s.length - lastSpace <= 9) {
                    th.innerHTML =
                        s.slice(0, lastSpace) + "<br>" + s.slice(lastSpace + 1);
                } else {
    ...
    function computeTotals(rows) {
        ...
        for (const r of rows) {
            for (const it of window.EQUIPMENT)
                totals[it.key] += Number(r.eq[it.key]) || 0;
        }
        return totals;
    }
